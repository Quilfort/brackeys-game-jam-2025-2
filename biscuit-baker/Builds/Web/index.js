    ...
    var GodotAudio = {
      ...
      SampleNodeBus: class SampleNodeBus {
        ...
        constructor(bus) {
          if (bus === null) {
            console.warn(
              "Attempted to create SampleNodeBus with null bus - creating dummy bus"
            );
            this._channelSplitter = GodotAudio.ctx.destination;
            this._channelMerger = GodotAudio.ctx.destination;
      ...
      SampleNode: class SampleNode {
        ...
        getSampleNodeBus(bus) {
          if (bus === null) {
            console.warn(
              "Attempted to get sample node bus for null bus - skipping"
            );
            return {
              setVolume: function () {},
              getInputNode: function () {
                return GodotAudio.ctx.destination;
              },
            };
          }
      ...
      Bus: class Bus {
        ...
        static getBus(index) {
          if (index < 0 || index >= GodotAudio.buses.length) {
            console.warn(`Invalid bus index "${index}" - returning null`);
            return null;
          }
